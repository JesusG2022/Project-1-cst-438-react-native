...
const initializeDatabase = async () => {
  try {
    ...
    await db.execAsync(`
      ...
      CREATE TABLE IF NOT EXISTS Posts_last (
        PostId INTEGER PRIMARY KEY, -- Primary key for posts
        UserId INTEGER, -- Foreign key referencing User_last table
        Date DATE, -- Date of the post
        text_quote VARCHAR(1000), -- Content of the post
        FOREIGN KEY (UserId) REFERENCES User_last(UserId) -- Establish relationship with User_last table
      )
    `);

    // Add Title column if it doesn't exist (for existing databases)
    try {
      await db.execAsync(`ALTER TABLE Posts_last ADD COLUMN Title VARCHAR(255)`);
      console.log('Title column added to Posts_last table');
    } catch (error) {
...
export const getAllUsers = async () => {
  try {
    ...
    const result = await db.getAllAsync('SELECT Username, Bio FROM User_last');
...
export const getPostsByUsername = async (username: string) => {
  try {
    const db = await dbPromise;
    // Get the user's ID
    const user = await db.getFirstAsync<{ UserId: number }>('SELECT UserId FROM User_last WHERE Username = ?', [username]);
...
export const getPostsByUserId = async (userId: number) => {
  try {
    const db = await dbPromise;
    const posts = await db.getAllAsync(
      'SELECT PostId, Date, Title, text_quote FROM Posts_last WHERE UserId = ? ORDER BY PostId DESC',
      [userId]
    );
    return posts; // Array of { PostId, Date, Title, text_quote }
  } catch (error) {
...
export const searchPosts = async (query: string) => {
  try {
    const db = await dbPromise;
    const posts = await db.getAllAsync(
      `SELECT Date, text_quote FROM Posts_last WHERE text_quote LIKE ?`,
